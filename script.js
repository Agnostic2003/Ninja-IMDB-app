    ...
    for(let i = 0; i<currentMovieStack.length; i++){
        ...
        <div class="movie-title-container">
            ...
            <div class="rating-container">
                <img src="https://res.cloudinary.com/dir9xnqbl/image/upload/v1678175025/IMDB/rating_button_gcswdu.png" alt="">
        ...
        <button onclick="${actionForButton}(this)" class="add-to-favourite-button text-icon-button" data-id="${currentMovieStack[i].id}" >
            <img src="https://res.cloudinary.com/dir9xnqbl/image/upload/v1678183476/IMDB/ddd.favourite_jelygs.png">
...
function favourite(element){
    ...
    for(let i = 0; i< currentMovieStack.length; i++){
        if(currentMovieStack[i].id == id){
            let favouriteMoviesAnand = JSON.parse(localStorage.getItem("favouriteMoviesAnand"));
            
            if(favouriteMoviesAnand == null){
                favouriteMoviesAnand = [];
            }

            favouriteMoviesAnand.unshift(currentMovieStack[i]);
            localStorage.setItem("favouriteMoviesAnand", JSON.stringify(favouriteMoviesAnand));

            showAlert(currentMovieStack[i].title + " added to your favourite ")
            return;
        }
    }
}
...
goToFavouriteButton.addEventListener('click', ()=>{
    let favouriteMoviesAnand = JSON.parse(localStorage.getItem("favouriteMoviesAnand"));
    if(favouriteMoviesAnand == null || favouriteMoviesAnand.length < 1){
        showAlert("you don't have any favourite movie");
        return;
    }

    currentMovieStack = favouriteMoviesAnand;
    renderList("remove");
})
...
function remove(element){
    ...
    for(let i = 0; i<favouriteMoviesAnand.length; i++){
        if(favouriteMoviesAnand[i].id == id){
            continue;
        }
        newFavouriteMovies.push(favouriteMoviesAnand[i]);
    }
    
    localStorage.setItem("favouriteMoviesAnand", JSON.stringify(newFavouriteMovies));
    ...
    movieDetailCard.innerHTML = `
        ...
        <div class="detail-movie-title">
            ...
            <div class="detail-movie-rating">
                <img src="https://res.cloudinary.com/dir9xnqbl/image/upload/v1678175025/IMDB/rating_button_gcswdu.png">
